...
import com.aprendizaje.item.clientes.ProductoClienteRest;
import com.aprendizaje.item.models.Item;
import com.aprendizaje.item.models.Producto;

@Service("item-service-feign")
public class ItemServiceFeign implements ItemService {
	...
	public List<Item> findAll() {
		return productoClienteRest.listar().stream().map(p -> new Item(p, 1))
				.collect(Collectors.toList());
	}

	@Override
	public Item findById(Long id, Integer cantidad) {
		var producto = productoClienteRest.detalle(id);
